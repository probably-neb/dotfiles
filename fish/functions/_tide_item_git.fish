function _tide_item_git
	...
	if set -l git_icon_out (string match -i '*gitlab*' (git config --get remote.origin.url))
		set -Ux git_icon \uf296
	else if set -l git_icon_out (string match -i '*github*' (git config --get remote.origin.url))
		set -Ux git_icon \uf09b
	end
	
    _tide_print_item git $_tide_location_color $git_icon' ' (set_color white; printf %s $location
